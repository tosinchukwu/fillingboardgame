...
DROP POLICY IF EXISTS "verified_results readable by anyone" ON public.verified_results;
create policy "verified_results readable by anyone"
  on public.verified_results for select
  to anon, authenticated
  using (true);

DROP POLICY IF EXISTS "verified_results writable by service role only" ON public.verified_results;
...
-- Policies (made idempotent)
DROP POLICY IF EXISTS "function runs readable by authenticated" ON public.switchboard_function_runs;
create policy "function runs readable by authenticated"
  on public.switchboard_function_runs for select
  to authenticated
  using (true);

DROP POLICY IF EXISTS "function runs writable by service role only" ON public.switchboard_function_runs;
