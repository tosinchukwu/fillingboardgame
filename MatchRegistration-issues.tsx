...
export function MatchRegistration() {
  ...
  const [successMatchType, setSuccessMatchType] = useState<boolean>(true);
  ...
  useEffect(() => {
    if (receipt && txHash) {
      if (receipt.status === 'success') {
        ...
        setSuccessMatchType(formData.isCasual);
  ...
  useEffect(() => {
    if (successMatchId) {
      const timeout = setTimeout(() => {
        ...
        setSuccessMatchType(true);
      }, 10000);
      return () => clearTimeout(timeout);
    }
  }, [successMatchId]);

  // Generate random numeric match ID
  const generateMatchId = () => {
    const randomBytes = Array.from(crypto.getRandomValues(new Uint8Array(4)))
      .map(b => b.toString(16).padStart(2, '0'))
      .join('');
    setFormData((prev) => ({ ...prev, matchId: randomBytes }));
    setError(null);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use numeric ID directly (as hex string)
      const matchIdHex = formData.matchId.toLowerCase();
      if (!/^[a-f0-9]{8}$/.test(matchIdHex)) {
        throw new Error('Match ID must be 8 hexadecimal characters (e.g., a1b2c3d4)');
      }

      console.log('[v0] Submitting match with ID:', matchIdHex);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="text-gray-300 font-medium block mb-2">Match ID (Auto-Generated Numeric)</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3.5 rounded-xl bg-slate-900/60 border border-slate-700/50 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/50 font-mono text-sm"
            ...
            <button
              ...
              disabled={isCreating || isConfirming || !!successMatchId}
              className="px-4 py-3.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              title="Auto-generate a random numeric match ID"
            >
              <Zap className="w-4 h-4" />
              <span className="hidden sm:inline">Auto</span>
            </button>
          </div>
          <p className="text-xs text-gray-500 mt-1">Format: 8 hexadecimal characters (e.g., a1b2c3d4)</p>
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-green-100/70">
                  {successMatchType ? '[CASUAL] – no prizes' : '[OFFICIAL] – with prizes'}
                </div>
              </div>
            </div>
          </div>
